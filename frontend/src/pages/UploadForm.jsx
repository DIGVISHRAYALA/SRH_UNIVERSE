...
const UploadForm = () => {
  ...
  const handleCreateRoom = () => {
    if (!team1.trim() || !team2.trim())
      return alert("Enter both team names!");
  ...
  // if (!isAuthenticated) {
  //   return (
  //     <div className="upload-form login">
  //       <h2>Admin Login</h2>
  //       <input
  //         type="password"
    ...
    <div className="upload-form">
      ...
      {activeSection === "videos" && (
        <>
          ...
          <h2>Uploaded Videos</h2>
          {uploadedVideos.map(video => (
            <div key={video._id} className="uploaded-section">
              <p><strong>{video.title}</strong></p>
              <a href={`${API_BASE}/videos/${video._id}/download`} download>Download</a>
              <p>Downloads: {video.downloadCount}</p>
              <p>Uploaded: {new Date(video.uploadedAt).toLocaleString()}</p>
              <button onClick={() => handleDeleteVideo(video._id)} className="danger">Delete</button>
            </div>
          ))}
        </>
      )}
      ...
      {activeSection === "articles" && (
        <>
          <h2>Upload Article</h2>
          <input value={articleTitle} onChange={e => setArticleTitle(e.target.value)} placeholder="Article Title" />
          <textarea value={articleContent} onChange={e => setArticleContent(e.target.value)} placeholder="Article Content" />
          <input value={articleTitleTelugu} onChange={e => setArticleTitleTelugu(e.target.value)} placeholder="Telugu Title" />
          <textarea value={articleContentTelugu} onChange={e => setArticleContentTelugu(e.target.value)} placeholder="Telugu Content" />
          <input type="file" accept="image/*" onChange={e => setArticleImage(e.target.files[0])} />
          <button onClick={handleArticleUpload}>Upload Article</button>
          ...
          <h2>Uploaded Articles</h2>
          {articles.map(article => (
            <div key={article._id} className="uploaded-section">
              <h4>{article.title}</h4>
              {article.titleTelugu && <h5>{article.titleTelugu}</h5>}
              {article.image && <img src={`${API_BASE}${article.image}`} alt={article.title} />}
              <p>{article.content}</p>
              {article.contentTelugu && <p>{article.contentTelugu}</p>}
              <button onClick={() => handleDeleteArticle(article._id)} className="danger">Delete</button>
            </div>
          ))}
        </>
      )}
      ...
      {activeSection === "rooms" && (
        <>
          <h2>Create Chat Room</h2>
          <input placeholder="Team 1 Name" value={team1} onChange={e => setTeam1(e.target.value)} />
          <input placeholder="Team 2 Name" value={team2} onChange={e => setTeam2(e.target.value)} />
          <button onClick={handleCreateRoom}>Create Room</button>
          ...
          {rooms.map(r => (
            <div key={r._id} className="room-item">
              <span>{r.title}</span>
              <button onClick={() => handleDeleteRoom(r._id)} className="danger">Delete</button>
            </div>
          ))}
        </>
      )}
